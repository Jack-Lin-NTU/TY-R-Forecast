...
class TyDataset(Dataset):
    ...
    def __init__(self, ty_list_file, root_dir, train=True, train_num=11, input_frames=5,
        ...
        if with_grid:
            self.gird_x, self.gird_y = np.meshgrid(np.arange(1,args.I_shape[0]+1),np.arange(1,args.I_shape[0]+1))
        if train:
        ...
        for i in self.idx_list.index:
            if idx > self.idx_list.loc[i,"The ending idx"]:
                ...
                for j in range(self.input_frames):
                    file_time=dt.datetime.strftime(self.idx_list.loc[i,"The starting time"]+dt.timedelta(minutes=10*(idx_tmp+j))
                                                          ,format="%Y%m%d%H%M")
