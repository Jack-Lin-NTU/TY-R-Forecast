...
from src.utils.GRUs_hparams import TRAJGRU_HYPERPARAMs
...
def train_epoch(model, dataloader, optimizer, args):
    ...
    print('[{:s}] Training Process: Ave_Loss = {:.2f}'.format(total_loss))
    print('[{:s}] Time spend: {:.1f} min'.format(time_step))
...
def eval_epoch(model, dataloader, args):
    ...
    print('[{:s}] Validating Process: {:d}, Loss = {:.2f}'.format(total_idx, total_loss))
...
if __name__ == '__main__':
    ...
    P = TRAJGRU_HYPERPARAMs(args)
    model = Model(P.n_encoders, P.n_forecasters, P.gru_link_size, \
                P.encoder_input_channel, P.encoder_downsample_channels, P.encoder_gru_channels, \
                P.encoder_downsample_k, P.encoder_downsample_s, P.encoder_downsample_p, P.encoder_gru_k, \
                P.encoder_gru_s, P.encoder_gru_p, P.encoder_n_cells, P.forecaster_input_channel, \
                P.forecaster_upsample_channels, P.forecaster_gru_channels, P.forecaster_upsample_k, \
    ...
    for epoch in range(args.max_epochs):
        ...
        print('[{:s}] Epoch {:03d}, Learning rate: {}'.format(epoch+1, lr))
