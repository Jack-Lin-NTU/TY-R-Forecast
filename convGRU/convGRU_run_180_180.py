...
from convGRU.convGRU_model import model
from convGRU.args_tools import args, createfolder
...
def train(net, trainloader, testloader, result_name, max_epochs=50, loss_function=BMSE,
    ...
    for epoch in range(max_epochs):
        ...
        for i, data in enumerate(trainloader,0):
            ...
            labels = labels[:,:,60:120,60:120].clone()

            # outputs.shape = [4, 18, 60, 60]
            outputs = net(inputs)
            outputs = outputs[:,:,60:120,60:120].clone()
...
def test(net, testloader, loss_function=BMSE, device=args.device):
    ...
    with torch.no_grad():
        for i, data in enumerate(testloader,0):
            ...
            labels = labels[:,:,60:120,60:120]
            outputs = net(inputs)[:,:,60:120,60:120]
            outputs = outputs.view(outputs.shape[0], -1)
            labels = labels.view(labels.shape[0], -1)
            loss += loss_function(outputs, labels)
...
def run(result_name, channel_factor, input_frames, output_frames, with_grid=args.input_with_grid,
    ...
    decoder_upsample_k = [4,4,5]
    decoder_upsample_s = [2,2,3]
...
if __name__ == "__main__":
    ...
    if args.input_with_grid:
        result_dir=os.path.join(args.result_dir,
                                'convGRU_i{:d}_o{:d}_c{:d}'.format(input_frames,output_frames,channel_factor),
                                'I{:d}_F{:d}'.format(args.I_shape[0],180))
    else:
        result_dir=os.path.join(args.result_dir,
                                'convGRU_i{:d}_o{:d}_c{:d}'.format(input_frames,output_frames,channel_factor),
                                'I{:d}_F{:d}_no_grid_info'.format(args.I_shape[0],180))
