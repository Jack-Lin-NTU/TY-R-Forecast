    ...
    def __getitem__(self, idx):
        ...
        for i in self.idx_list.index:
            if idx > self.idx_list.loc[i, 'The ending idx']:
                ...
                file_time1 = dt.datetime.strftime(self.idx_list.loc[i,'The starting time'], format='%Y%m%d%H%M')
                file_time2 = dt.datetime.strftime(self.idx_list.loc[i,'The starting time']+dt.timedelta(minutes=10*(self.input_frames+self.target_frames)), format='%Y%m%d%H%M')
                ...
                self.sample = {'inputs': input_data, 'targets': target_data, 'ty_infos': ty_infos}
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        # numpy data: x_tsteps X H X W
        # torch data: x_tsteps X H X W
        return {'inputs': input_data, 'targets': target_data, 'ty_infos': ty_infos}
