...
def output_files(args, typhoon_list):
    '''
    Arguments:
...
def output_files_p(args):
    return output_files(*args)

def parallel_output_files(args):
    from multiprocessing import Pool
    typhoon_list = sorted(os.listdir(args.radar_compressed_data_folder))
    chunks = [typhoon_list[i::5] for i in range(5)]
    pool = Pool(processes=5)
    job_args = [(args, x) for x in chunks]
    # breakpoint()
    pool.map(output_files_p, job_args)
...
def data_statistics():
    ...
    '''
    # Set path
    measures = pd.DataFrame(np.ones((4,3))*10, index=['max_value','min_value','mean_value','std_value'], columns=sorted(os.listdir(args.radar_wrangled_data_folder)))
    ...
    for i in sorted(os.listdir(args.radar_wrangled_data_folder)):
        for j in sorted(os.listdir(os.path.join(args.radar_wrangled_data_folder, i))):
            ...
            if measures.loc['max_value', i] < np.max(tmp_data):
                measures.loc['max_value', i] = np.max(tmp_data)
            if measures.loc['min_value', i] > np.min(tmp_data):
                measures.loc['min_value', i] = np.min(tmp_data)
            tmp_mean += np.mean(tmp_data)/100
        measures.loc['mean_value', i] = tmp_mean/len(os.listdir(os.path.join(args.radar_wrangled_data_folder, i)))*100
    tmp = 0
    for i in sorted(os.listdir(args.radar_wrangled_data_folder)):
        for j in sorted(os.listdir(os.path.join(args.radar_wrangled_data_folder, i))):
            tmp_data = pd.read_pickle(os.path.join(args.radar_wrangled_data_folder, i , j), compression=args.compression).values
            tmp += np.mean((tmp_data - measures.loc['mean_value', i])**2)
        measures.loc['std_value', i] = np.sqrt(tmp/len(os.listdir(os.path.join(args.radar_wrangled_data_folder, i))))
